...
parser.add_argument(
    "--stopping_fold_id", type=str, default="1", help="The fold id to use for early stopping"
)
...
def censoring_weights(df, model_type = 'KM'):
    ...
    censoring_model.fit(df.query('is_train==1').event_time_10yr, 1.0*~df.query('is_train==1').event_indicator)
...
if __name__ == "__main__":
    ...
    cohort = cohort.assign(is_train = lambda x: np.where((x.fold_id != config_dict.get('stopping_fold_id')) & (x.fold_id != "test") 
                                                         & (x.fold_id != "eval"),
                                                         1, 0))
...
#     print(config_dict.get('batch_size'))
    ...
    if args.run_evaluation:
        ...
        predict_dict = model.predict(data.loaders_dict_predict, phases=['val', 'test', 'eval'])
