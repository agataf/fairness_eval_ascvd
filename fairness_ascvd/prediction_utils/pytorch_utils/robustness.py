...
from fairness_ascvd.prediction_utils.pytorch_utils.models import (
    TorchModel,
    FeedforwardNet,
)
...
class GroupDROModel(TorchModel):
    ...
    def compute_constraints(
        ...
        for the_group in group.unique():
            ...
            try:
                constraint_values[the_group] = constraint_values[
                    the_group
                ] + constraint_metric_fn(
...
class GroupDROAdjustedLoss(GroupDROModel):
    def __init__(self, *args, cohort_df=None, **kwargs):
        super().__init__(*args, **kwargs)

        self.group_adjustments = self.compute_group_adjustments(cohort_df=cohort_df)
    ...
    def compute_group_adjustments(self):
        raise NotImplementedError

    def get_constraint_baselines(self, exact_constraints=False):
        if exact_constraints:
            return torch.tensor(
                self.group_adjustments, dtype=torch.float, device=self.device
            )
        else:
            return torch.zeros(self.config_dict.get("num_groups")).to(self.device)


class GroupDROSizeAdjusted(GroupDROAdjustedLoss):
    def get_default_config(self):
        config_dict = super().get_default_config()
        update_dict = {"use_exact_constraints": True, "adjustment_scale": 1}
        return {**config_dict, **update_dict}

    def compute_group_adjustments(self, cohort_df):
        if self.config_dict.get("weight_var_name") is None:
            adjustment = (
                (cohort_df.groupby("group_id").agg("size") / cohort_df.shape[0])
                .pipe(np.sqrt)
                .pipe(lambda x: x * self.config_dict["adjustment_scale"])
                .sort_index()
                .values
            )
        else:
            weight_var_name = self.config_dict.get("weight_var_name")
            adjustment = (
                (
                    cohort_df.groupby("group_id").agg(
                        weight_sum=(weight_var_name, "sum")
                    )
                    / cohort_df[weight_var_name].sum()
                )
                .pipe(np.sqrt)
                .pipe(lambda x: x * self.config_dict["adjustment_scale"])
                .sort_index()
        ...
        print("Computed adjustment: {}".format(adjustment))
        return adjustment


class GroupDROSizeAdjustedReciprocal(GroupDROSizeAdjusted):
    def compute_group_adjustments(self, cohort_df):
        """
        Computes the reciprocal adjustment of the GroupDROSizeAdjusted
        This one is consistent with Sagawa et al. 2020
        """
        if self.config_dict.get("weight_var_name") is None:
            adjustment = (
                (cohort_df.groupby("group_id").agg("size") / cohort_df.shape[0])
                .pipe(np.sqrt)
                .pipe(lambda x: self.config_dict["adjustment_scale"] * (1 / x))
                .sort_index()
                .values
            )
        else:
...
class GroupDROBaselinedLoss(GroupDROModel):
    """
    Computes an adjustment to the loss on the basis of the entropy of the label
    """

    def constraint_metric(self, outputs, labels, sample_weight=None):
        return baselined_loss(outputs, labels, sample_weight=sample_weight)

    def constraint_metric_exact(self, outputs, labels, sample_weight=None):
        return baselined_loss(outputs, labels, sample_weight=sample_weight)


class GroupDROAUC(GroupDROModel):
    def constraint_metric(self, outputs, labels, sample_weight=None):

        return 1 - roc_auc_score_surrogate(
            outputs=outputs,
            labels=labels,
            sample_weight=sample_weight,
...
class GroupDROAUCProxy(GroupDROModel):
    def constraint_metric_exact(self, outputs, labels, sample_weight=None):
        return 1 - roc_auc_score_surrogate(
            outputs=outputs,
            labels=labels,
